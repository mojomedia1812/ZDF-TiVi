...
GITHUB_LATEST_RELEASE_URL = "https://api.github.com/repos/mojomedia1812/ZDF-TiVi/releases/latest"
USER_AGENT = "plugin.video.zdf-tivi-update-check"
...
def validate_update_zip(zip_path, expected_version):
    ...
    with archive:
        ...
        if addon_id != ADDON_ID:
            raise UpdateError("Update-ZIP gehört zu einem anderen Add-on: {0}".format(addon_id))
    ...
    with zipfile.ZipFile(zip_path) as archive:
        for member in archive.infolist():
            ...
            if not os.path.normcase(target_path).startswith(target_root):
                raise UpdateError("Update-ZIP würde außerhalb des Add-on-Verzeichnisses schreiben")
